...
/* ==========================================================================
   OPŠTI DELOVI
   ========================================================================== */
/* Dugme „Nastavi preko Google-a".
   Znak je Google-ov, u njegovim bojama i sa fill-om — CW.icon() crta sve
   jednom bojom i linijom, pa ovaj ne može kroz njega. */
CW.c.googleButton = function (label) {
  return '<button class="btn btn--secondary btn--lg btn--full" type="button" data-act="google-auth">' +
    ...
    '</svg>' +
    '<span>' + CW.esc(label || 'Nastavi preko Google-a') + '</span>' +
  '</button>';
};
...
CW.ui.bind = function () {
  ...
  document.addEventListener('click', function (ev) {
    ...
    switch (act) {
      ...
      /* Google-ov znak sme samo u originalnim bojama — zato nije u CW.ICONS,
         koji sve crta jednom bojom. */
      case 'google-auth':
        ev.preventDefault();
        if (!CW.sb || !CW.sb.enabled) {
          CW.toast({ type: 'error', title: 'Prijava trenutno ne radi' });
          break;
        }
